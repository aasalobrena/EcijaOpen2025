...
Define("CanScrambleEvent", And(CanStaff(), Or(IsInTop25Psych({1, Event}), And(Or((EventId({1, Event}) == "333bf"), (EventId({1, Event}) == "333oh")), IsInTop25Psych(_333)))))
Define("CanScrambleEventRelaxed", In(Slice(Sort(Persons(And(CanStaff(), CompetingIn({1, Event}))), If(Or((EventId({1, Event}) == "333bf"), (EventId({1, Event}) == "333oh")), PsychSheetPosition(_333), PsychSheetPosition({1, Event}))), 0, {2, Number})))
...
Define("DefaultAssignmentSets", Concat(DelegatesSet(), OrganizersSet(), DeLaTorreSet(), UkuSet(), TopCompetitorsSet({1, Round}), EveryoneSet()))
...
Define("DefaultJobs",
	[
		Job("runner", 3, eligibility=CanStaff()),
		Job("judge", CompetitorsPerGroup({1, Round}), eligibility=CanStaff()),
...
Define("FinalJobs",
	[
		...
		Job("judge", CompetitorsPerGroup({1, Round}), eligibility=CanStaff()),
		Job("runner", 2, eligibility=CanStaff())
	]
)